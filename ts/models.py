...
class LogThermostat(models.Model):
    ...
    hum = models.FloatField("Humidity", blank=True, null=True)
    ...
    def __str__(self):
        return f"{self.time} | State: {'On' if self.on else 'Off'} | t: {self.temp}" \
               f"{' | co2:' + str(self.co2) if self.co2 else ''}" \
               f"{' | humidity:' + str(self.hum) if self.hum else ''}"
